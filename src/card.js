...
export default class Card extends RenderObject {
  ...
  constructor(cardData, isYours) {
    ...
    const frontTexture = cardData.code
      ? Scene.textureLoader.load(`/assets/en_us/img/cards/${cardData.code}.png`)
      : backTexture;
  ...
  moveTo(x, y, z = 1, durationMs = 250) {
    console.log(
      `Card ${this.data.id || "(no id)"}/${
        this.data.code
      } is moving to ${x}, ${y}, ${z}. ${
        durationMs === 0
          ? "This will happen immediately"
          : `This will take ${durationMs} ms.`
      }`
    );

    this.addAnimation().add(
      new AnimationEffect(
    ...
    return this.addAnimation().add(
      new AnimationEffect(Easing.easeInOutQuad, this, "z", z, durationMs)
    );
  }

  showFront(delay = 0, durationMs = 250) {
    if (this.data.code == "") return this.addAnimation(); // If we can't show the front, don't.
  ...
  static async loadThreeJSShader() {
    if (this.fragShader && this.vertShader) return;
  ...
  get rotation() {
    return this.quad.rotation;
  }
  get width() {
    return this.quad && this.quad.parameters && this.quad.geometry
      ? this.quad.geometry.parameters.width
      : 1;
  }
  get height() {
    return this.quad && this.quad.parameters && this.quad.geometry
      ? this.quad.geometry.parameters.height
      : 1;
  }
  ...
  get showingFront() {
    return Math.abs(Math.PI - this.rotation.y) < Math.abs(this.rotation.y);
  }

  get destroyAmount() {
    return this.material.uniforms.destroyAmount.value;
  }
  set destroyAmount(value) {
    this.material.uniforms.destroyAmount.value = value;
  }
}
