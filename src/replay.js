...
export default class Replay {
  ...
  static skipToActionIndex(actionIterator) {
    ...
    for (let i = actionIterator; i < Replay.actions.length; i++) {
      Replay.actions[i]._hasStarted = false;
      Replay.actions[i].done = undefined;
      Replay.actions[i].animation = undefined;
      Replay.actions[i].animations = [];
      Replay.actions[i].finalAnimation = null;
    }
  ...
  static getActions(data) {
    ...
    switch (actionName) {
      ...
      case "draw":
        ...
        if (this.resolveFirstDraws(data)) return [];
      ...
      case "enemy_play":
        return [
          new SetupCardsForAttackAction(defaultIsYou, {
            code: data.code,
            id: data.id
          })
        ];
      ...
      case "fight":
        ...
        // 	}[];
        // }
        return [new ResolveFight(true, true, data.matchups)];
  ...
  static getExistingAction(name) {
    for (let actionData of this.actions)
      if (actionData.name == name) return actionData;
  ...
  static play(actions) {
    ...
    if (typeof actions !== "number") {
      ...
      Scene.reset();
      Replay.players = Replay.players || [
        new PlayerSide(true),
        new PlayerSide(false)
      ];
    ...
    if (Replay.currentAction) {
      ...
      if (Replay.currentAction.hasStarted) {
        if (Replay.currentAction.isDone(timeMs)) {
          if (Replay.currentAction.isYou) Replay.timeLastYourAction = timeMs;
          else Replay.timeLastTheirAction = timeMs;
          Replay._currentActionIterator++; // Go to next action

          if (Replay.currentAction) {
            console.log(
              `Current action done. Will start ${Replay.currentAction.name} soon.`
            );
          } else {
    ...
    if (Replay.onFrame) Replay.onFrame(Replay._currentActionIterator, timeMs);
  ...
  static identifyEnemyHandCard(cardIdent) {
    ...
    const cardArrays = this.actions
      .filter(a => !a.isYou)
      .map(a => a.deckCardData);
  ...
  static resolveFirstDraws(data) {
    ...
    if (!show || !replace) {
      console.error(
        "Can't find the show or replace mulligan card actions, but we're drawing a card? Huh?!"
      );
